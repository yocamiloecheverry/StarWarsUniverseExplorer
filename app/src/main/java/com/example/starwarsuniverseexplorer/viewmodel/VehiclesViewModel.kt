...
class VehiclesViewModel(private val repo: StarWarsRepository) : ViewModel() {
    ...
    // 4 Usages
    private val _state = MutableStateFlow<NetworkResult<List<VehicleDto>>>(NetworkResult.Loading)
    val state: StateFlow<NetworkResult<List<VehicleDto>>> = _state
    ...
    private fun load() {
        viewModelScope.launch {
            ...
            try {
                _state.value = NetworkResult.Success(data = repo.getVehicles().results)
            } catch (e: Exception) {
